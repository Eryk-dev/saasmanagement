...
try {
  for (const width of [1440,1920,390]) {
    ...
    assert.equal(await page.locator('.today-script').getByRole('heading',{name:'Roteiro',exact:true}).count(),0);
    assert.equal(await page.locator('.today-step').count(),0);
    const shortcut=await page.locator('.today-script-shortcuts').boundingBox();
    const columns=await page.locator('.today-script-columns').boundingBox();
    const history=await page.locator('.today-script-history').boundingBox();
    assert.ok(shortcut.y+shortcut.height<=columns.y+1);
    assert.ok(Math.abs(shortcut.width-columns.width)<2);
    assert.ok(history.y>=columns.y+columns.height-1);
    ...
    await page.getByRole('button',{name:'Fechar atividade',exact:true}).click();
