...
// Dados: aba só no celular; no desktop a coluna lateral fica sempre à vista.
const showData=async d=>{const b=d.getByRole('button',{name:'Dados',exact:true});if(await b.count())await b.click();await d.getByRole('complementary',{name:'Dados do atendimento'}).waitFor();};
...
try{
 for(const width of process.env.REVIEW_FLOWS?[]:[1440,1920]){
  ...
  await app.locator('.support-row').first().click();await detail(app).getByLabel('Resposta ao cliente',{exact:true}).waitFor();await h.capture(app,`detail-wide-${width}`);
 ...
 await openFirst(p);await showData(detail(p));await detail(p).getByRole('complementary',{name:'Dados do atendimento'}).waitFor();
 ...
 await p.getByRole('button',{name:'Abrir ticket',exact:true}).click();const newTicket=p.getByRole('dialog',{name:'Novo ticket'});await newTicket.getByLabel('Assunto',{exact:true}).fill('Ticket fictício da revisão');await newTicket.getByRole('button',{name:'Abrir ticket',exact:true}).click();await detail(p).getByLabel('Resposta ao cliente',{exact:true}).waitFor();await showData(detail(p));p.once('dialog',d=>d.dismiss());await detail(p).getByRole('button',{name:'Apagar ticket'}).click();assert.equal(await detail(p).count(),1);p.once('dialog',d=>d.accept());await detail(p).getByRole('button',{name:'Apagar ticket'}).click();await detail(p).waitFor({state:'hidden'});await p.close();
 ...
 for(const method of ['ticketCreate','ticketMessage','ticketDelete']){
  ...
  if(method==='ticketCreate'){
   ...
   else{await showData(detail(slow));slow.once('dialog',d=>d.accept());await detail(slow).getByRole('button',{name:'Apagar ticket'}).click();assert.ok(await detail(slow).getByRole('button',{name:'Apagar ticket'}).isDisabled());}
