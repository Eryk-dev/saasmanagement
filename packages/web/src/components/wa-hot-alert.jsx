...
//
// O mesmo pop-up serve DOIS avisos, que são o mesmo problema em dois momentos:
//   hot  — o lead RESPONDEU numa conversa aberta (responder na hora)
//   lead — o lead ACABOU DE ENTRAR pelo formulário e ninguém falou com ele
//          (fazer o 1º toque enquanto ele ainda está com o assunto na cabeça)
// Conversa aberta ganha da entrada nova: quem já está falando com a gente
// espera menos que quem ainda nem foi abordado.

const SNOOZE_MS = 5 * 60_000;

// Lead novo interrompe alguém por 2h (o mesmo SLA de 1º contato que o funil
// declara no estágio Novo lead). Depois disso ele não some: está na fila do Meu
// dia, que é onde o atraso é cobrado, sem pop-up de ontem para quem chega hoje.
const LEAD_WINDOW_MS = 2 * 3_600_000;
const kindOf = (a) => (a?.kind === "lead" ? "lead" : "hot");
...
export function WaHotAlert({ onOpenThread, onOpenLeadWhatsapp, onOpenLeadCard }) {
  ...
  const visible = alerts
    .filter((a) => {
      ...
      // Lead novo tem prazo de validade (ver LEAD_WINDOW_MS); conversa aberta
      // fica até alguém responder ou resolver.
      if (kindOf(a) === "lead" && now - new Date(a.at || 0).getTime() > LEAD_WINDOW_MS) return false;
      return true;
    })
    .sort((x, y) => (kindOf(x) === kindOf(y) ? 0 : kindOf(x) === "hot" ? -1 : 1));
  ...
  // Lead novo não tem conversa pra "responder daqui": o aviso serve pra alguém
  // PEGAR o lead. Quem clica assumiu, então o alerta é resolvido pra todo mundo
  // (dois SDRs não caem no mesmo lead) e a tela vai pro 1º toque.
  function take(go) {
    api.waAlertDone(cur.id).catch(() => {});
    setAlerts((a) => a.filter((x) => x.id !== cur.id));
    go?.(cur);
  }
      ...
      <div style={{ width: 480, maxWidth: "94vw", background: "var(--bg-1)", border: "1px solid var(--line-1)", borderRadius: "var(--r-3)", boxShadow: "0 24px 80px -24px rgba(0,0,0,.45)", overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 16px" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
            ...
            {isLead && cur.company && <span className="dim" style={{ fontSize: 12.5 }}>{cur.company}</span>}
          ...
          <div style={{ marginTop: 10, padding: "10px 12px", background: "var(--bg-2)", border: "1px solid var(--line-1)", borderRadius: "var(--r-2)", fontSize: 13.5, lineHeight: 1.45, color: "var(--fg-1)" }}>
            {cur.text || (isLead ? "Acabou de enviar o formulário." : "…")}
          </div>

          {isLead ? (
            <div style={{ display: "flex", gap: 8, marginTop: 12, alignItems: "center" }}>
              <button onClick={() => take(onOpenLeadWhatsapp)}
                style={{ ...btn, background: "var(--wa-brand)", borderColor: "transparent", color: "var(--wa-brand-fg)", fontWeight: 700 }}>
                Chamar no WhatsApp
              </button>
              <button onClick={() => take(onOpenLeadCard)} style={btn}>Abrir o card</button>
              <span style={{ flex: 1 }} />
              <button onClick={done} style={{ ...btn, borderColor: "transparent", background: "transparent" }} title="fecha o aviso pra todo mundo (ex.: já falei com essa pessoa)">resolvido</button>
