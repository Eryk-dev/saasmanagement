    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
      <div style={{ padding: "12px 24px", borderBottom: "1px solid var(--line-1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", gap: 2 }}>
          {[["open","Open · tracked"],["builder","Builder"]].map(([k,l]) => (
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button style={chromeBtnStyleSmall}><span style={{ fontSize: 11 }}>+ from template</span></button>
          <button style={{ ...chromeBtnStyleSmall, borderColor: "var(--accent-line)", color: "var(--accent)" }}><span style={{ fontSize: 11 }}>+ new proposal</span></button>
        </div>
      </div>
      ...
      {tab === "open" && (
        <div style={{ flex: 1, overflow: "auto", padding: "16px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 14 }}>
            <SmallStat k="Sent (this mo)" v="14" d={+3} />
            <SmallStat k="Open rate"      v="79%" d={+0.04} dU="pp" />
            <SmallStat k="Time-in-section (median)" v="3:21" sub="Pricing section" />
            <SmallStat k="Acceptance"     v="34%" d={+0.06} dU="pp" />
          </div>
          <div style={{ border: "1px solid var(--line-1)", borderRadius: "var(--r-3)", background: "var(--bg-1)" }}>
            <div className="mono" style={{ display: "grid", gridTemplateColumns: "1.6fr 80px 100px 90px 1fr 70px 90px",
              ...
              <span>Proposal</span><span style={{ textAlign: "right" }}>Amount</span><span>Status</span>
              <span>Opens</span><span>Sections viewed</span><span>Last</span><span>Rep</span>
            </div>
            {PROPOSALS.map(p => (
  ...
  return (
    <div style={{ flex: 1, display: "grid", gridTemplateColumns: "200px 1fr 280px", minHeight: 0 }}>
      ...
      <div style={{ borderRight: "1px solid var(--line-1)", background: "var(--bg-1)", padding: "14px 12px", overflow: "auto" }}>
        <div className="mono" style={{ fontSize: 10, color: "var(--fg-4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Sections</div>
        {sections.map(s => (
        ...
        <div className="mono" style={{ fontSize: 10, color: "var(--fg-4)", letterSpacing: "0.08em", textTransform: "uppercase", margin: "16px 0 8px" }}>Blocks</div>
        {["Heading","Rich text","Table","Pricing table","Image","Variables","Approval"].map(b => (
      ...
      <div style={{ overflow: "auto", padding: 28, background: "var(--bg-0)" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", background: "var(--bg-1)", border: "1px solid var(--line-1)", borderRadius: "var(--r-3)", padding: "40px 48px", minHeight: "100%" }}>
          <div className="mono dim" style={{ fontSize: 10, letterSpacing: "0.08em" }}>PROPOSAL · {active.toUpperCase()}</div>
          <h2 style={{ fontSize: 24, fontWeight: 500, margin: "8px 0 6px", letterSpacing: "-0.01em" }}>
            {{ Cover: "Helios Media — Q2 Performance Engagement",
               Approach: "Our approach to Helios Media",
               Pricing: "Investment & terms",
               Terms: "Terms and conditions",
               Appendix: "Appendix" }[active]}
          </h2>
          <div className="mono dim" style={{ fontSize: 12 }}>prepared by Mika Kessler · {new Date().toDateString()}</div>
          ...
          {active === "Pricing" && (
            <div style={{ marginTop: 22 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px 100px", gap: 1, background: "var(--line-1)", border: "1px solid var(--line-1)", borderRadius: "var(--r-2)", overflow: "hidden" }}>
                {["Module","Seats","Unit","Total"].map(h => <div key={h} style={{ padding: "10px 12px", background: "var(--bg-inset)", fontSize: 11, color: "var(--fg-3)", fontFamily: "var(--mono)", letterSpacing: "0.05em", textTransform: "uppercase" }}>{h}</div>)}
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16, fontFamily: "var(--mono)" }}>
                <div style={{ textAlign: "right" }}>
                  <div className="mono dim" style={{ fontSize: 11 }}>annual</div>
              ...
              <p className="mono dim" style={{ fontSize: 11, marginTop: 24, lineHeight: 1.5 }}>
                ▸ Dynamic variable: deal.amount inherits from Helios Media (d1). Edits here update the deal record.
              </p>
            </div>
          )}
          {active !== "Pricing" && (
            <p style={{ marginTop: 22, fontSize: 13, color: "var(--fg-2)", lineHeight: 1.6 }}>
              <span className="mono dim">{"{{client.name}}"}</span> is exploring an expansion of their performance marketing
              footprint. We've outlined a phased engagement that brings their attribution model under one roof while
              keeping <span className="mono dim">{"{{client.team_lead}}"}</span>'s team in control of campaign decisions.
            </p>
          )}
        </div>
      </div>
      ...
      <div style={{ borderLeft: "1px solid var(--line-1)", background: "var(--bg-1)", padding: "14px 14px", overflow: "auto" }}>
        <div className="mono" style={{ fontSize: 10, color: "var(--fg-4)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>Send & tracking</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
          {[
            { l: "Recipient",       v: "ceo@helios.media" },
            { l: "Unique link",     v: "✓ track per-section dwell" },
            { l: "Expires",         v: "in 14 days" },
            { l: "Auto-advance",    v: "on accept → Closed Won" },
            { l: "Slack notify",    v: "#deals · @mika" },
          ].map(r => (
        ...
        <button style={{ width: "100%", padding: "10px 14px", background: "var(--accent)", color: "var(--accent-fg)", borderRadius: "var(--r-2)", fontSize: 13, fontWeight: 500 }}>Send proposal</button>
        <div className="mono dim" style={{ fontSize: 10, marginTop: 12 }}>Once sent, opens, time-in-section and accept/decline events flow back to the deal timeline.</div>
      </div>
    </div>
  );
}
