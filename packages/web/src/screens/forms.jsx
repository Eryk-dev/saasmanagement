      ...
      <section className="forms-responses-card">
        ...
        <div className="tbl-x"><div className="forms-response-table">
          ...
          {rows.map(s=>{const level=grade(s),lead=leads.get(String(s.lead));return <React.Fragment key={s.id}>
            <div className="forms-response-row"><button aria-expanded={open===s.id} onClick={()=>setOpen(open===s.id ? null : s.id)}><strong>{mapped(s,"name") || Object.values(s.answers || {})[0] || "Sem nome"}</strong><small>{mapped(s,"email") || mapped(s,"phone") || "—"}</small></button><span>{mapped(s,"company") || "—"}</span><span className="tnum">{lead?.accounts || s.answers?.accounts || "—"}</span><span title={lead ? leadTier(lead).label : "Sem classificação"} style={{color:level ? GRADE_STYLE[level]?.ink : "var(--fg-3)",fontWeight:700}}>{level || "—"}{lead && leadTier(lead).legacy ? " L" : ""}</span><span>{[s.variant,s.pain && `[${s.pain}]`].filter(Boolean).join(" · ") || "—"}</span><time>{date(s.createdAt)}</time></div>
