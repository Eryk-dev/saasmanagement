...
test("screenForRequest: mapa por prefixo + escritas administrativas", () => {
  assert.equal(screenForRequest("GET", "/api/expenses/summary/leverads"), "expenses");
  assert.equal(screenForRequest("GET", "/api/marketing/leverads"), "metrics");
  assert.equal(screenForRequest("POST", "/api/leads/l1/proposal"), "pipeline");
  assert.equal(screenForRequest("GET", "/api/customers"), "customers");
  assert.equal(screenForRequest("GET", "/api/products"), null);           // catálogo é leitura livre
  assert.equal(screenForRequest("PATCH", "/api/products/leverads"), "settings");
  assert.equal(screenForRequest("POST", "/api/auth/users"), "settings");
