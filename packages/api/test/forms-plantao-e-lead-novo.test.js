...
//   1. no PLANTÃO (fora do expediente: noite de dia útil e fim de semana) o
//      botão do "obrigado" manda o lead pro número de quem está de plantão,
//      e não pro número comercial, onde não tem ninguém;
...
// Relógio de Brasília (UTC-3), injetado na rota: o plantão depende da hora, e
// teste preso ao relógio real passaria ou falharia conforme a hora em que roda.
// Em ago/2026: 05=qua, 07=sex, 08=sáb, 10=seg.
const brt = (iso) => new Date(`${iso}-03:00`);
const PLANTAO = { enabled: true, phone: "5541995063622" };

async function buildApp({ duty = null, at = "2026-08-05T10:00", product = {}, forms = [], leads = [] } = {}) {
  const repo = makeMemRepo();
  await repo.create("forms", { ...FORM });
  for (const f of forms) await repo.create("forms", f);
  await repo.create("products", { id: "leverads", name: "LeverAds", ...product, ...(duty ? { duty } : {}) });
  for (const l of leads) await repo.create("leads", l);
  const app = Fastify();
  registerRoutes(app, repo, { forms: { now: () => brt(at) } });
  await app.ready();
  return { app, repo };
}
...
test("no plantão, o número do plantonista vence até o número escrito no form", async () => {
  const { app } = await buildApp({ duty: PLANTAO, at: "2026-08-05T22:00" }); // quarta à noite
...
test("a virada acontece na hora certa, nos dois sentidos", async () => {
  const numeroEm = async (at) => {
    const { app } = await buildApp({ duty: PLANTAO, at });
    const pub = (await app.inject({ method: "GET", url: "/public/forms/fo_test" })).json();
    await app.close();
    return pub.thanks.whatsapp;
  };
  const PLANTONISTA = "5541995063622", COMERCIAL = "5541936183835";
  assert.equal(await numeroEm("2026-08-05T17:59"), COMERCIAL,   "quarta 17h59, time ainda na operação");
  assert.equal(await numeroEm("2026-08-05T18:00"), PLANTONISTA, "quarta 18h, o plantão assume");
  assert.equal(await numeroEm("2026-08-06T07:59"), PLANTONISTA, "quinta 7h59, ainda plantão");
  assert.equal(await numeroEm("2026-08-06T08:00"), COMERCIAL,   "quinta 8h, time de volta");
  assert.equal(await numeroEm("2026-08-08T12:00"), PLANTONISTA, "sábado, fim de semana inteiro");
});

test("no expediente, nada muda: continua o número do form", async () => {
  const { app } = await buildApp({ duty: PLANTAO, at: "2026-08-05T10:00" });
  const pub = (await app.inject({ method: "GET", url: "/public/forms/fo_test" })).json();
...
test("produto sem plantão configurado nunca troca de número", async () => {
  const { app } = await buildApp({ at: "2026-08-08T12:00" }); // nem no sábado
  const pub = (await app.inject({ method: "GET", url: "/public/forms/fo_test" })).json();
  assert.equal(pub.thanks.whatsapp, "5541936183835");
  await app.close();
});
...
test("plantão é por produto: o form de outro SaaS não é afetado", async () => {
  const { app, repo } = await buildApp({ duty: PLANTAO, at: "2026-08-05T22:00" });
