...
- Recomendações de Publicidade/Formulários usam `Modal` em portal, com
  foco/Escape, bloqueio durante execução e botões que nomeiam a ação.
...
- **Detalhe com conversa é modal de duas colunas, não painel lateral (14/09/2026, Suporte → Tickets).** O painel à direita espremia o quadro e empurrava a conversa pra baixo de oito campos. `Modal` largo (1080, altura fixa com rolagem interna): cabeçalho com nº · origem, assunto editável e uma linha de estado (status, prioridade, responsável, SLA em risco); coluna principal com abas Conversa/Atividade e a resposta fixa embaixo (o único botão primário); coluna lateral de 320px em `--bg-inset` com seções por kicker (Atendimento, Prazos, Cliente, Anexos) e campos rótulo à esquerda. Rascunho na resposta trava véu e Esc (`fechavel={false}`) e o ✕ confirma o descarte. Abaixo de 900px empilha: dados, conversa, resposta. Ver `screens/tickets/detail.jsx`
- **Seletor de uma opção em modal/painel é `SelectPopover`, não `<select>` nativo (14/09/2026, Suporte).** O nativo abre a lista desenhada pelo sistema operacional (fonte, cor e altura do navegador) ao lado do `UserPicker`, que já abria no Popover — no mesmo modal, dois desenhos de dropdown. `components/select-popover.jsx`: gatilho na medida do `.inp` com chevron, lista no `Popover` com ✓ na opção atual, ponto de 6px (`tone`) pra status/prioridade, `hint` à direita, ↑↓ Enter Esc e busca acima de 8 opções. **Não colocar dentro de `<label>`**: o clique na opção sobe até o label e reabre a lista — use `div` + kicker. Aplicado no detalhe e no novo ticket; telas antigas migram quando forem tocadas
- **Checkbox, liga/desliga e campo de horas desenhados (14/09/2026, Configurações de SLA): `components/form-controls.jsx`.** `Checkbox` (caixa 16px, `--accent` marcada, `role="checkbox"`; com children vira linha clicável), `Switch` e `SwitchRow` (configuração com título + explicação à esquerda e o switch à direita, linha inteira clicável — é o desenho pra toggle de configuração; checkbox fica pra marcar itens de uma lista) e `HoursInput` (duração guardada em minutos, texto com vírgula e a unidade "h" dentro; o `type="number"` trazia o spinner do navegador e não aceitava "1,5"). O `Toggle` local de metrics.jsx é a mesma peça, migra quando a tela for tocada
