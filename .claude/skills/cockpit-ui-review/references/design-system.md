...
- Carregamento inicial/navegação: `AppStartup` + `ScreenTransition` usam o splash LeverAds aprovado em `design/splash-loading-crm/`. Logo 120 × 130 na entrada/Visão Geral, 64 × 70 nas outras telas, órbita de 800ms e saída suave (450ms/180ms). O conteúdo monta oculto e inerte; aparece quando as consultas GET iniciais terminam. Menu/topo continuam disponíveis na navegação. Após 12s, oferecer saída da espera. Respeitar `prefers-reduced-motion`; sem percentuais fictícios. SSE/polling não reabre o splash.
